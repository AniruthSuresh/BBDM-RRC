...
class BBDMRunner(DiffusionBaseRunner):
    ...
    def initialize_model(self, config):
        if config.model.model_type == "BBDM":
            ...
            bbdmnet = LatentBrownianBridgeModel(config.model).to(
    ...
    def load_model_from_checkpoint(self):
        ...
        if self.config.model.only_load_latent_mean_std:
            if (
                self.config.model.__contains__("model_load_path")
                and self.config.model.model_load_path is not None
            ):
                states = torch.load(
                    self.config.model.model_load_path, map_location="cpu"
                )
        else:
        ...
        if self.config.model.normalize_latent:
            if states is not None:
                self.net.ori_latent_mean = states["ori_latent_mean"].to(
                    self.config.training.device[0]
                )
                self.net.ori_latent_std = states["ori_latent_std"].to(
                    self.config.training.device[0]
                )
                self.net.cond_latent_mean = states["cond_latent_mean"].to(
                    self.config.training.device[0]
                )
                self.net.cond_latent_std = states["cond_latent_std"].to(
                    self.config.training.device[0]
                )
            else:
    ...
    def get_latent_mean_std(self):
        ...
        def calc_mean(batch, total_ori_mean=None, total_cond_mean=None):
            ...
            total_ori_mean = (
                x_mean if total_ori_mean is None else x_mean + total_ori_mean
            )
            ...
            total_cond_mean = (
                ...
                else x_cond_mean + total_cond_mean
            )
            return total_ori_mean, total_cond_mean

        def calc_var(
            batch,
            ori_latent_mean=None,
            ...
            x_latent = self.net.encode(x, cond=False, normalize=False)
            x_cond_latent = self.net.encode(x_cond, cond=True, normalize=False)
            x_var = ((x_latent - ori_latent_mean) ** 2).mean(
                axis=[0, 2, 3], keepdim=True
            )
            total_ori_var = x_var if total_ori_var is None else x_var + total_ori_var

            x_cond_var = ((x_cond_latent - cond_latent_mean) ** 2).mean(
                axis=[0, 2, 3], keepdim=True
            )
            total_cond_var = (
                x_cond_var if total_cond_var is None else x_cond_var + total_cond_var
            )
        ...
        for train_batch in tqdm(train_loader, total=len(train_loader), smoothing=0.01):
            ...
            #     break
            batch_count += 1
            total_ori_mean, total_cond_mean = calc_mean(
                train_batch, total_ori_mean, total_cond_mean
            )
        ...
        for train_batch in tqdm(train_loader, total=len(train_loader), smoothing=0.01):
            ...
            #     break
            batch_count += 1
            total_ori_var, total_cond_var = calc_var(
                train_batch,
    ...
    def loss_fn(self, net, batch, epoch, step, opt_idx=0, stage="train", write=True):
        ...
        loss, additional_info = net(x, x_cond, x_mask)
        if write and self.is_main_process:
            ...
            if additional_info.__contains__("recloss_noise"):
                self.writer.add_scalar(
                    f"recloss_noise/{stage}", additional_info["recloss_noise"], step
                )
            if additional_info.__contains__("recloss_xy"):
                self.writer.add_scalar(
                    f"recloss_xy/{stage}", additional_info["recloss_xy"], step
                )
        return loss
    ...
    def sample(self, net, batch, sample_path, stage="train"):
        ...
        reverse_one_step_path = make_dir(
            os.path.join(sample_path, "reverse_one_step_samples")
        )
        ...
        image_grid = get_image_grid(
            sample, grid_size, to_normal=self.config.data.dataset_config.to_normal
        )
        im = Image.fromarray(image_grid)
        im.save(os.path.join(sample_path, "skip_sample.png"))
        if stage != "test":
            self.writer.add_image(
                f"{stage}_skip_sample", image_grid, self.global_step, dataformats="HWC"
            )
        ...
        if stage != "test":
            self.writer.add_image(
                f"{stage}_condition", image_grid, self.global_step, dataformats="HWC"
            )

        image_grid = get_image_grid(
            x.to("cpu"), grid_size, to_normal=self.config.data.dataset_config.to_normal
        )
        im = Image.fromarray(image_grid)
        im.save(os.path.join(sample_path, "ground_truth.png"))
        if stage != "test":
            self.writer.add_image(
                f"{stage}_ground_truth", image_grid, self.global_step, dataformats="HWC"
            )
    ...
    def sample_to_eval(self, net, test_loader, sample_path):
        ...
        result_path = make_dir(
            os.path.join(sample_path, str(self.config.model.BB.params.sample_step))
        )
            ...
            for j in range(sample_num):
                ...
                for i in range(batch_size):
                    ...
                    if j == 0:
                        ...
                        save_single_image(
                            gt, gt_path, f"{x_name[i]}.png", to_normal=to_normal
                        )
                    if sample_num > 1:
                        ...
                        save_single_image(
                            result, result_path, f"{x_name[i]}.png", to_normal=to_normal
                        )

    # @torch.no_grad()
    # def sample_to_eval(self, net, test_loader, sample_path):

    #     print("Using the updated one now !!!")
    ...
    #         (_, _), (x_cond, x_cond_name) = test_batch
    ...
    #                 # New naming convention: batch index + image index + sample number
    #                 condition_filename = f'cond_batch{batch_idx}_img{i}.png'
    #                 result_filename = f'output_batch{batch_idx}_img{i}_sample{j}.png'

    #                 if j == 0:
    #                     save_single_image(condition, condition_path, condition_filename, to_normal=to_normal)

    #                 if sample_num > 1:
    #                     result_path_i = make_dir(os.path.join(result_path, f'batch_{batch_idx}_img_{i}'))
    #                     save_single_image(result, result_path_i, result_filename, to_normal=to_normal)
    #                 else:
    #                     save_single_image(result, result_path, result_filename, to_normal=to_normal)
