...
class BrownianBridgeModel(nn.Module):
    def __init__(self, model_config):
        ...
        self.max_var = (
            model_params.max_var if model_params.__contains__("max_var") else 1
        )
    ...
    def register_schedule(self):
        ...
        if self.skip_sample:
            if self.sample_type == "linear":
                ...
                steps = np.linspace(
                    start=0, stop=self.num_timesteps, num=self.sample_step + 1
                )
                steps = (
    ...
    def forward(self, x, y, context=None):
        ...
        (
            ...
            device,
            img_size,
        ) = *x.shape, x.device, self.image_size
        assert (
    ...
    def q_sample_loop(self, x0, y):
        imgs = [x0]
        for i in tqdm(
            range(self.num_timesteps), desc="q sampling loop", total=self.num_timesteps
        ):
    ...
    def p_sample_grad(self, x_t, y, context, i, clip_denoised=False):
        b, *_, device = *x_t.shape, x_t.device
        if self.steps[i] == 0:
            t = torch.full(
                (x_t.shape[0],), self.steps[i], device=x_t.device, dtype=torch.long
            )
            objective_recon = self.denoise_fn(x_t, timesteps=t, context=context)
            ...
            n_t = torch.full(
                (x_t.shape[0],), self.steps[i + 1], device=x_t.device, dtype=torch.long
            )
    ...
    def p_sample(self, x_t, y, context, i, clip_denoised=False):
        return self.p_sample_grad(x_t, y, context, i, clip_denoised=clip_denoised)
    ...
    def p_sample_loop(self, y, context=None, clip_denoised=True, sample_mid_step=False):
        return self.p_sample_loop_grad(y, context, clip_denoised, sample_mid_step)
